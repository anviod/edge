<template>
  <a-card v-for="item in items" :key="item.id" class="northbound-card" hoverable>
    <template #title>
      <div class="card-title-row">
        <span class="protocol-tag">SPB</span>
    ...
    <template #extra>
      <a-space size="small">
        <a-tooltip content="帮助文档">
          <a-button type="text" size="mini" @click="showHelp(item)">
    ...
    <div class="card-info-list">
      ...
      <div class="info-row">
        <span class="info-label"><icon-folder :size="14" /> Group ID</span>
        <span class="info-value text-ellipsis">{{ item.group_id }}</span>
      </div>
      <div class="info-row">
        <span class="info-label"><icon-idcard :size="14" /> Node ID</span>
  ...
  <SparkplugHelpDialog
    v-model="helpVisible"
    :group-id="currentItem?.group_id || ''"
    :node-id="currentItem?.node_id || ''"
  />
</template>
