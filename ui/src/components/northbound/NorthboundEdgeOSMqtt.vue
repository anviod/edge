  ...
  <a-card v-for="item in items" :key="item.id" class="northbound-card" hoverable>
    <template #title>
      <div class="card-title-row">
        ...
        <span class="protocol-sub">MQTT</span>
    ...
    <template #extra>
      <a-space size="small">
        ...
        <a-tooltip content="删除">
          <a-button type="text" size="mini" status="danger" @click="$emit('delete', 'edgeos-mqtt', item.id)">
    ...
    <div class="card-info-list">
      ...
      <div class="info-row">
        <span class="info-label"><icon-send :size="14" /> QoS</span>
        <span class="info-value">{{ item.qos }}</span>
      </div>
    </div>
...
<script setup>
import { IconQuestionCircle, IconSettings, IconBarChart, IconDelete, IconCloud, IconIdcard, IconSend, IconCopy, IconRobot } from '@arco-design/web-vue/es/icon'
