<template>
  <a-modal
    ...
    :title="isEdit ? '编辑 edgeOS(NATS) 配置' : '新增 edgeOS(NATS) 配置'"
    :width="1000"
    @ok="saveSettings"
    :ok-loading="loading"
    unmount-on-close
    ...
    <a-tabs v-model:active-key="activeTab" type="line" class="industrial-tabs">
      <a-tab-pane key="basic">
        ...
        <a-form :model="form" layout="horizontal" :label-col-props="{ span: 5 }" :wrapper-col-props="{ span: 19 }" class="industrial-form">
          ...
          <a-form-item label="JetStream">
            <a-switch v-model="form.jetstream_enabled" type="round" />
          </a-form-item>
...
const saveSettings = async () => {
  ...
  try {
    ...
    await request.post('/api/northbound/edgeos-nats', dataToSave)
    showMessage('edgeOS(NATS) 配置已保存', 'success')
